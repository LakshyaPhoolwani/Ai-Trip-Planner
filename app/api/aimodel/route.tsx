...
export async function POST(request: NextRequest) {
  ...
  const decision = await aj.protect(request, { userId:user?.primaryEmailAddress?.emailAddress??"<user_id>", requested: isFinal ? 5 : 0 }); // Deduct 5 tokens from the bucket
  // console.log("Arcjet decision", decision);
  
  //@ts-ignore
  if(decision?.reason?.remaining ==0 && !hasPremiumAccess){
    return NextResponse.json({ 
      response: "No Free Credits Remaining",
      ui:"limit"
  });
  }
